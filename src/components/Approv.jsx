...
function Approv() {
  ...
  const [approvalStatus, setApprovalStatus] = useState(
    localStorage.getItem("approvalStatus") === "true" // Check if there's a stored value
      ? true
      : false
  );
  ...
  const fetchDataAndCheckApprovalStatus = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/v1/event/approved/${id}`
      );
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`http://localhost:5000/api/v1/event/guest/${id}`)
      .then((response) => {
  ...
  const handleApproval = async (id) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:5000/api/v1/event/approve/${id}`
      );
  ...
  const handleSaveClick = () => {
    axios
      .patch(`http://localhost:5000/api/v1/event/${id}`, newdata.getAllEvents)
      .then((response) => {
  ...
  const handleAnomalies = () => {
    ...
    axios
      .patch(`http://localhost:5000/api/v1/event/${id}`, dataToUpdate)
      .then((response) => {
        if (response.status === 200) {
